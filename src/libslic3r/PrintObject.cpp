...
bool visualize = this->model_object()->name == "VISUALIZE";
if (this->model_object()->name == "TEST" || visualize) {
        double angle = -Geometry::deg2rad(45.);
        for (Layer* layer : m_layers) {
            std::cout << "\n\nLAYER_Z : " << layer->print_z << std::endl;
            ...
            for (LayerRegion* region : layer->m_regions) {
                ...
                for (ExtrusionEntity* ee : region->fills.entities) {
                    ...
                    if (! ee->is_collection()) {
                        std::cout << "not a collection!" << std::endl;
                        std::terminate();
                    }
                    if (ee->role() != erBridgeInfill) {
                        auto eec = dynamic_cast<ExtrusionEntityCollection*>(ee);
                        temp = eec->as_polylines();
                        for (Polyline& p : temp)
                            p.rotate(angle);
                        infill.insert(infill.end(), temp.begin(), temp.end());

                        if (visualize) {
                            if (! eec->empty()) {
                                auto role = eec->entities.front()->role();
                                eec->clear();
                                extrusion_entities_append_paths(eec->entities, std::move(temp), role, 0.5, 0.5, 0.15);
                            }
                        }
                    }
                }
                ...
                for (ExtrusionEntity* ee : region->fills.entities) {
                    if (ee->role() == erBridgeInfill) {
                        ...
                        if (visualize) {
                            //auto role = ee->role();
                            //eec->clear();
                            //extrusion_entities_append_paths(eec->entities, bridge, role, 0.5, 0.5, 0.15);
                        }
                        ...
                        double spacing = lines.size() > 1 ? (lines.back().x - lines.front().x) / (lines.size() - 1) : 0.;
                            ...
                            for (size_t i=1; i<p.points.size(); ++i) {
                                ...
                                while (idx_end >= idx_start && idx_end >= 0) {
                                    ...
                                    if (std::isnan(slope)) { // first run
                                        ...
                                        std::cout << lines.size() << " " << idx_start << " " << idx_end << std::endl;
...

                        {
                            ...
                            Polylines polylines;
                            for (VertLine& line : lines) {
                                polylines.emplace_back(Polyline(Point(line.x, line.y_bottom), Point(line.x, line.y_top)));
                                if (! visualize)
                                    polylines.back().rotate(-angle);
                            }
                            extrusion_entities_append_paths(eec->entities, polylines, erBridgeInfill, 0.5, 0.5, 0.15);
                        }
                    }
                }
